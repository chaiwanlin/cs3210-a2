...
def main(sig_file, inputs):
	...
	if cached:
		...
		if p.returncode != 0:
			print(f"sequential run failed with exit code {p.returncode}!")
			sys.exit(1)

		cached_lines = list(map(lambda x: x.strip(), p.stdout.decode().splitlines()))
		if not cached_lines[-1].startswith("ELAPSED"):
			print(f"sequential run failed because the output didn't start with 'ELAPSED' as expected!")
			print(f"Actual output: {cached_lines}")
			sys.exit(1)
	...
	print(f"")
	print(f"parallel speedup (for bonus - assuming this is on a xgpg node): {parallel_time_ns / cuda_runtime_ns:.3f}x")
	print(f"")
